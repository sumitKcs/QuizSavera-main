...
const QuizHeader = ({ menu_items }) => {
    ...
    return (
        <header className="bg-[#10a19d] flex flex-col justify-start items-start lg:flex-row lg:justify-start px-4 py-2 gap-3 lg:gap-8 text-white">
            <div className="flex gap-2 w-full lg:w-fit">
                {/* mobile menu icon */}
                <span className={`${!toggleMenu ? "flex" : "hidden"} lg:hidden`} onClick={handleMobileMenu}>&#9776;</span>
                {/* brand name */}
                <a href="/" className="flex-1 text-center lg:text-left"><span className="text-xl font-extrabold">QuizSavera</span></a>
            </div>

            {/* menu items */}
            <nav className={`flex justify-start lg:justify-end items-center flex-wrap gap-2 text-[14px] md:text-[16px] md:gap-4`}>
